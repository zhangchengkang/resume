<template>
    <div id="workexprience">
      <el-row>
        <div class="page-title">工作经历</div>
      </el-row>
      ...
      <div class="table-container">
        <el-row>
          <div class="form-title"><i class="el-icon-edit"></i>工作经历</div>
        </el-row>
          ...
          <el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%" border stripe
            ...
            <el-table-column align="center" prop="companyName" label="公司名">
              <template slot-scope="scope">
                <el-input v-if="showEdit[scope.$index]" v-model="scope.row.companyName" size="small" placeholder="请输入内容"></el-input>
                <span v-if="!showEdit[scope.$index]">{{scope.row.companyName}}</span>
              </template>
            </el-table-column>
             <el-table-column align="center" prop="position" label="职位">
              <template slot-scope="scope">
                <el-input v-if="showEdit[scope.$index]" v-model="scope.row.position" size="small" placeholder="请输入内容"></el-input>
                <span v-if="!showEdit[scope.$index]">{{scope.row.position}}</span>
              </template>
            </el-table-column>
             <el-table-column align="center" prop="time" label="任职时间段">
              <template slot-scope="scope">
             ...
             <el-table-column align="center" prop="description" label="职责描述">
              <template slot-scope="scope">
...
export default {
  ...
  methods: {
    // 初始化数据
    async initTableData() {
      this.axios.post("/workexperience/findall", {"userId":this.getCookieValue("userId")}).then(res => {
    ...
    handleDelete(index, row) {
      this.axios.get("/workexperience/delete/"+row.id).then(res => {
      });
    ...
    handleEdit(index, row) {
      ...
      if (editBtn.innerHTML == "编辑") {
        ...
        this.axios.post("/workexperience/save", row).then(res => {
