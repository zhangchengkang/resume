<template>
    <div id="sellmyself">
      <el-row>
        <div class="page-title">扩展信息</div>
      </el-row>
      <el-row class="breadcrumb">
        <el-breadcrumb separator-class="el-icon-arrow-right">
          <el-breadcrumb-item :to="{ path: '/Home/homaepage' }">首页</el-breadcrumb-item>
          <el-breadcrumb-item>扩展信息</el-breadcrumb-item>
        </el-breadcrumb>
      </el-row>
      <div class="table-container">
        <el-row>
          <div class="form-title"><i class="el-icon-edit"></i>扩展信息</div>
        </el-row>
        <div class="form-container">
          ...
          <el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%" border stripe
            ...
            <el-table-column align="center" prop="info" label="信息">
              <template slot-scope="scope">
                <el-input v-if="showEdit[scope.$index]" v-model="scope.row.info" size="small" placeholder="请输入内容"></el-input>
                <span v-if="!showEdit[scope.$index]">{{scope.row.info}}</span>
              </template>
            </el-table-column>
...
export default {
  ...
  methods: {
    // 初始化数据
    async initTableData() {
      this.axios.post("/extendinfo/findall", {"userId":this.getCookieValue("userId")}).then(res => {
        this.tableData.push(res.data.data[0].info)
      });
    },
    ...
    handleDelete(index, row) {
      this.axios.get("/extendinfo/delete/"+row.id).then(res => {
      });
    ...
    handleEdit(index, row) {
      ...
      if (editBtn.innerHTML == "编辑") {
        ...
        this.axios.post("/extendinfo/save", row).then(res => {
