<template>
    <div id="skills">
      <el-row>
        <div class="page-title">职业技能</div>
      </el-row>
      <el-row class="breadcrumb">
        <el-breadcrumb separator-class="el-icon-arrow-right">
          <el-breadcrumb-item :to="{ path: '/Home/homaepage' }">首页</el-breadcrumb-item>
          <el-breadcrumb-item>职业技能</el-breadcrumb-item>
        </el-breadcrumb>
      </el-row>
      <div class="table-container">
        <el-row>
          <div class="form-title"><i class="el-icon-edit"></i>职业技能</div>
        </el-row>
        <div class="form-container">
          ...
          <el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%" border stripe
            ...
            <el-table-column align="center" prop="skill" label="技能清单">
              <template slot-scope="scope">
                <el-input v-if="showEdit[scope.$index]" v-model="scope.row.skill" size="small" placeholder="请输入内容"></el-input>
                <span v-if="!showEdit[scope.$index]">{{scope.row.skill}}</span>
              </template>
            </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleDelete(index, row) {
      this.axios.get("/vocationalskills/delete/" + row.id).then(res => {});
    ...
    handleEdit(index, row) {
      ...
      if (editBtn.innerHTML == "编辑") {
        ...
        this.axios.post("/vocationalskills/save", row).then(res => {});
